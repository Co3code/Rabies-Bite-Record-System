<?php
    ...
    if (empty($errors)) {
        ...
        $stmt = mysqli_prepare($conn,
            "UPDATE patients SET fullname=?, age=?, sex=?, address=?, contact=? WHERE id=?");
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            mysqli_stmt_close($stmt); // Close even if failed
        }
    }
...
<nav class="navbar navbar-custom px-3">
    <span class="navbar-brand text-white">Edit Patient</span>
